...
## 综合题型三：利用“定积分的结果是一个数字”来求解某些待定函数的问题
...
!!! success
    ...
    $J_n$ 单调递减且有下界0，故极限存在。在递推关系中令 $n \to \infty$，得 $\lim_{n \to \infty} J_n = 0$。

    $$
    \lim_{n \to \infty} n J_n = \dfrac{1}{2} \lim_{n \to \infty} [n J_n + (n+2) J_{n+2}] = \dfrac{1}{2} \lim_{n \to \infty} \dfrac{n}{n+1} = \dfrac{1}{2}
    $$
